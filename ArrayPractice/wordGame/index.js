        ...
        document.getElementById('box_' + i).addEventListener('click', function (event) {
            ...
            if(arrayOfClientResponse.length === word.length){
                ...
                if(arrayOfClientResponse === word){
                    document.getElementById('statement').innerHTML = 'Winner Winner Chicken Dinner'
                }else{
                    document.getElementById('statement').innerHTML = 'Sorry, Better luck next time<br><br> Correct word is: '+'<span class=\'correctWord\'>'+word+'</span>'
                }
            }             
        })
...
let currentWord;
function hint(){
    document.getElementById('letter_0').value = currentWord[0]
    // console.log('Its working outside of for loop')
    for (let index = 0; index < currentWord.length; index++) {
        // console.log('Its working in for loop')
        if(document.getElementById('box_' + index).value === currentWord[0]){
            document.getElementById('box_' + index).classList.add('clientResponse')
            document.getElementById('box_' + index).attributes.isClicked.value = 'true'
            arrayOfClientResponse += currentWord[0]
            letterBoxId++
            break;
        }
